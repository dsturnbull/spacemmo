...
assemble(sasm_t *sasm, char *src_file)
{
    ...
    define_constant(sasm, "MMU",        IRQ_MMU);
...
push1(sasm_t *sasm, op_t op, void *data, size_t len)
{
    ...
    slog("push1 %016lx = op:%02x data:%016llx\n",
            sasm->ip - sasm->prog, op, *(uint64_t *)data);

    memcpy(sasm->ip++, &opcode, sizeof(uint8_t));
    for (size_t i = 0; i < len; i++) {
        slog("push1 %016lx = db:%02x\n",
...
{
    slog("define_variable, name:%s value:%016llx len:%08lx\n",
            name, value, len);
...
{
    slog("find_or_create_variable, name:%s\n", name);
    variable_t *variable;
...
add_variable_ref(sasm_t *sasm, variable_t *var, uint64_t addr, size_t reflen)
{
    ...
    ref->addr = addr;
    ref->len = reflen;
    slog("added ref for %s at %016llx\n", var->name, ref->addr);
...
{
    slog("define_constant, name:%s addr:%016llx\n", name, addr);

    variable_t *var = find_variable(sasm, name);

    if (var) {
        var->addr = sasm->ip - sasm->prog;
        slog("already found, new addr: %016llx\n", var->addr);
    } else {
        slog("not seen\n");
...
{
    slog("define_data, name:%s len:%08lx\n", name, len);
...
{
    slog("define_label, name:%s\n", name);
    variable_t *var = find_variable(sasm, name);
    if (var) {
        var->addr = sasm->ip - sasm->prog;
        slog("already found %s, new addr (%016llx) (%lu refs)\n",
                var->name, var->addr, var->refs_len);
    } else {
        slog("haven't seen %s, adding constant @ %016lx\n",
...
write_data(sasm_t *sasm)
{
    ...
    for (size_t i = 0; i < sasm->variables_len; i++) {
        ...
        if (var->len > 0) {
            slog("var: %s\n", var->name);
            if (var->data) {
                slog("writing %s ", var->data);
                memcpy(sasm->ip, var->data, var->len);
            } else {
                slog("writing %016llx ", var->value);
    ...
    for (size_t i = 0; i < sasm->variables_len; i++) {
        ...
        for (size_t j = 0; j < var->refs_len; j++) {
            ...
            slog("%s replacing ref at %016llx with %016llx (%08lx)\n",
                    var->name,
                    var->refs[j]->addr,
                    value,
                    var->refs[j]->len);
...
print_prog(sasm_t *sasm)
{
    ...
    //    for (int j = 0; j < 16; j++) {
    //        for (int k = 0; k < 8; k++) {
    //            //printf("%i", (sasm->prog[i + j] & (1 << k)) != 0);

    //            if ((sasm->prog[i + j] & (1 << k)) == 0)
    //                fprintf(stderr, ".");
    //            else
    //                fprintf(stderr, "o");
    //        }
